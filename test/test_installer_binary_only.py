...
def _run_installer(
    ...
    *,
    ...
    if existing_pipx_venv:
        ...
        # keys on, bin/kirocrew is the launcher pipx's symlink points at, and
        # pipx_metadata.json records any `pipx inject`ed packages.
        old = pipx_venvs / "kirocrew"
...
def test_pipx_injected_venv_is_installed_in_place(tmp_path: Path, signing_key: SigningKey) -> None:
    """A venv holding `pipx inject`ed packages is never moved aside: pipx's
    --force installs into the existing venv and keeps the injections, so the
    installer leaves that path exactly as it is today. The injected command
    survives, and no backup is ever created or deleted."""
    case = tmp_path / "case"
    result, _argv = _run_installer(
        case, signing_key, with_pipx=True, existing_pipx_venv=True, injected=True
    )
...
def test_pipx_injected_venv_failure_is_todays_behaviour(
    tmp_path: Path, signing_key: SigningKey
) -> None:
    """With injections present the transaction is off, so a failure is pipx's
    own (pre-existing) outcome: no restore wording, the plain failure."""
    case = tmp_path / "case"
    result, _argv = _run_installer(
        case,
        signing_key,
