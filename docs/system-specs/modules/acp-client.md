...
- **Pi gate files under a masked `run/`**. The enforced-adapter credential mask includes the crew `run/` directory (`sandbox_credential_targets` expands `_CREW_SECRET_LEAVES`), which is also where `_seal_pi_gate_extension` and `_ensure_pi_gate_launcher` write the verified gate source and launcher. After those two files exist, the pi arm of `_spawn` assigns `_pi_gate_sandbox_expose(adapter_expose, launcher, extension)` back onto `adapter_expose` so both the gate read-back `wrap_argv_async` and the later session wrap pass the same `extra_expose_files` tuple. That is a file-level read carve-out, not a directory unmask: `extra_hidden_dirs` still hides the rest of `run/`, and the arm never sets `extra_visible_dirs`. Without the carve-out the sandboxed read-back cannot open the sealed source it is required to report.
