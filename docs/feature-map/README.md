...
| Files in chat | Browse, attach and upload workspace files; search the rail by file NAME or by file CONTENTS, including the text inside Word/PowerPoint/Excel documents; preview one in the side panel — a .docx renders its headings, lists, tables and bold/italic runs, falling back to extracted plaintext; right-click a file row → Download saves it to the machine running the browser through the credential-gated download endpoint; the header carries the per-project quick actions — reveal the project directory, open it in the editor configured for folders (LINUX desktop app only: the handler is the `inode/directory` MIME binding, and `shell.openPath` resolves the path inside the call there, which is what lets the main process validate it — the row is withheld in a browser tab and on a macOS/Windows shell), and open a terminal already in it | Chat right panel → Files tab, Name/Content toggle below the search field; right-click a file row → **Download**; composer attach; Files tab header → reveal button and **Project actions** → **Open project in editor** (Linux desktop app) / **Open terminal in the project directory**; right-click a transcript folder chip → **Open folder in editor** (Linux desktop app) | `pages/chat/FileBrowserRail.tsx`, `pages/chat/FilesHomePanel.tsx`, `api/fileGrep.ts`, `components/FileRenderers.tsx`, `components/FilePathMenu.tsx`, `lib/electron.ts`, `pierre/PierreWorkspaceTreeImpl.tsx` | `handlers/files.py` | `GET /api/file-read`, `GET /api/file-office-preview`, `GET /api/file-download`, `POST /api/file-grep`, `POST /api/upload`, `POST /api/upload/file`; the editor launch is not an HTTP call — it is the `dashboard:open-dir` Electron IPC channel |
