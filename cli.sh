...
if command -v pipx >/dev/null 2>&1; then
  ...
  #
  # A venv with `pipx inject`ed packages is NOT moved aside. pipx's --force
  # installs INTO an existing venv (`--force-reinstall`, no --clear), so today
  # the injected packages and their exposed commands survive a reinstall; a
  # fresh venv would silently lack them and deleting the backup would lose
  # them for good. Such a venv keeps today's in-place install exactly -- the
  # injections are preserved, the (pre-existing) delete-on-failure stays.
  # The metadata is read with the interpreter; a missing or unreadable file
  # is treated as "may hold injections" so nothing is ever moved on a guess.
  _pipx_venv_has_no_injections() {
...
try:
    ...
    raise SystemExit(1)
raise SystemExit(0 if not meta.get("injected_packages") else 1)
' "$1/pipx_metadata.json"
  }
  _PIPX_VENV_BACKUP=""
  _PIPX_VENV="$(pipx environment --value PIPX_LOCAL_VENVS 2>/dev/null || true)"
  _PIPX_VENV="${_PIPX_VENV:+${_PIPX_VENV%/}/kirocrew}"
  if [ -n "$_PIPX_VENV" ] && [ -f "$_PIPX_VENV/pyvenv.cfg" ] && [ ! -L "$_PIPX_VENV" ] \
      && _pipx_venv_has_no_injections "$_PIPX_VENV"; then
