...
def build_shadow_venv(wheel_path: Path, shadow_dir: Path, stable_link: Path | None = None) -> None:
    ...
    try:
        ...
        # binary-only policy, pip's "No matching distribution found" is the
        # no-wheel verdict, so the refusal names the platform and the way out
        # rather than leaving the operator with pip's raw text.
...
def _no_wheel_message(missing: list[str]) -> str:
    """The operator-facing refusal for a host no wheel of ``missing`` can run on.

    Mirrors cli.sh's message so an install and its later update describe the
    same platform floor; adds the one fact that is specific to the update path:
    the opt-in is read from the gateway process's environment, so a host that
    installed with it has to carry it in the service unit for updates to honour it.
    """
    ...
    return (
        f"no prebuilt wheel for {', '.join(missing)} runs on this host ({host}). "
        "Kiro Crew installs prebuilt wheels only and never compiles a dependency, so "
        "the current dependency set needs a newer Linux (Amazon Linux 2023, RHEL/Rocky "
        "8+, Ubuntu 22.04+, Debian 12+) on x86_64/aarch64, or macOS. To compile on this "
        f"host instead, install a C/C++ toolchain and the -dev headers, then set "
