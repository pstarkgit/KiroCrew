...
export function usePinnedPrompt({ scrollerRef, requiresMountedHandoff = false }: UsePinnedPromptOptions) {
  ...
  /**
   * Jump back to the pinned prompt with one self-driven converging glide. A
   * host with every row mounted can omit `steer`; the live row rect supplies
   * the destination. A virtualized host supplies `steer`, which preserves its
   * current window for a far target and uses the height-index estimate until
   * the row mounts. Every frame then prefers live geometry, so row measurement
   * and the banner swap refine the landing. User scroll intent aborts the
   * glide, and owning each frame's write prevents follow-controller writes
   * from cancelling it.
   */
  ...
  const jumpToPinnedPromptInPlace = useCallback((
    ...
    const goal = (): number | null => {
      ...
      const rowTop = row
        ...
        : steer?.estimateRowTop(anchor) ?? null
